...
<details>
  <summary>Via Powershell terminal (as admin).</summary>
...
Now you can manage Windows applications and packages using Chocolatey in administrative shell,
check out a list of installed packages with `choco list --local`.
...
> For Windows 11 you can also use the [fine-tuning Sophia Script script from GitHub](https://github.com/farag2/Sophia-Script-for-Windows.git).
...
the no-brainer would be to link the settings to your JetBrains account, since no additional configuration is required.
...
You can set a rule for `wf.msc` by subnet and interface:
```powershell
...
For further security, create a preferences file.
...
- [x] Install __Chocolatey__, add a package list
- [ ] Make a screenshot of the IDE
